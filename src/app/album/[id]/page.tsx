...
import { Play, Pause, Clock, Calendar, Disc, Loader2, User, Music, Heart } from "lucide-react";
...
export default function AlbumPage() {
  ...
  const { playTrack, currentTrack, isPlaying, setHasInteracted, favorites, toggleFavorite } = usePlayerStore();
  ...
  return (
    <div className="flex flex-col pb-20 relative">
      ...
      <div
        className="absolute top-0 left-0 right-0 h-[500px] pointer-events-none opacity-40 transition-colors duration-1000"
        style={{
      ...
      {/* Header Section */}
      <div className="relative flex flex-col md:flex-row items-end gap-8 mb-10 pt-8 px-8 rounded-3xl z-10">
        <div className="w-64 h-64 flex-shrink-0 shadow-2xl rounded-2xl overflow-hidden relative group">
          <img
            src={albumCover}
            alt={album.title}
          ...
          <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition flex items-center justify-center">
            <button
              ...
              className="w-16 h-16 bg-cyan-500 text-black rounded-full flex items-center justify-center shadow-xl hover:scale-110 active:scale-95 transition"
            >
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm font-medium text-neutral-300">
        ...
        {album.tracks.map((track, index) => {
          ...
          return (
            <div
              ...
              className={`grid grid-cols-[16px_1fr_120px] gap-4 px-4 py-3 rounded-lg transition group cursor-pointer ${isCurrent ? "bg-neutral-800/60" : "hover:bg-neutral-800/30"
                }`}
            >
