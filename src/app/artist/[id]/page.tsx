...
export default function ArtistPage() {
  ...
  return (
    <div className="flex flex-col pb-20 -mt-8 -mx-8 md:-mt-12 md:-mx-12 relative">
      {/* Dynamic Background Gradient */}
      <div 
        className="absolute top-0 left-0 right-0 h-[800px] pointer-events-none opacity-30 transition-colors duration-1000 z-0"
        style={{
      ...
      {/* Header Banner */}
      <div className="relative h-[300px] md:h-[450px] overflow-hidden group z-10">
        {headerImage ? (
          <img 
            src={headerImage} 
            alt={artist.name}
          ...
          <div className="w-full h-full bg-neutral-800 flex items-center justify-center">
            <User className="w-32 h-32 text-neutral-700" />
          </div>
        )}
        ...
        <div className="absolute bottom-0 left-0 p-8 md:p-12 w-full">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-bold tracking-widest uppercase text-cyan-400">Verified Artist</span>
          </div>
      ...
      <div className="p-8 md:p-12 flex flex-col gap-12 relative z-10">
        {/* Top Tracks */}
        <section>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
              {artist.albums.map((album) => (
                <Link 
                  key={album.id} 
                  href={`/album/${album.id}`}
                  className="group flex flex-col gap-3 bg-neutral-900/40 p-4 rounded-xl border border-transparent hover:border-neutral-700 transition cursor-pointer"
                >
                  ...
                  <h3 className="font-bold text-white truncate group-hover:text-cyan-400 transition">{album.title}</h3>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
              {artist.singles.map((single) => (
                ...
                >
                  ...
                  <h3 className="font-bold text-white truncate group-hover:text-cyan-400 transition">{single.title}</h3>
        ...
        {artist.relatedArtists.length > 0 && (
          <section>
            <h2 className="text-2xl font-bold text-white mb-6">Fans Might Also Like</h2>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
              {artist.relatedArtists.map((ra) => (
                <Link 
                  key={ra.id} 
                  href={`/artist/${ra.id}`}
                  className="group flex flex-col items-center gap-3 bg-neutral-900/40 p-4 rounded-xl border border-transparent hover:border-neutral-700 transition cursor-pointer"
                >
                  <div className="w-32 h-32 rounded-full overflow-hidden shadow-lg bg-neutral-800">
                    <img 
                  ...
                  <h3 className="font-bold text-white text-center truncate w-full group-hover:text-cyan-400 transition">{ra.name}</h3>
